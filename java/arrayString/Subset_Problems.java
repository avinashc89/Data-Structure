...
public class Subset_Problems {
	...
	//
	public static void subset_That_Sums_To_N(int[] list, int index, int currSum, int goal, String result)
	{ 
		...
		for (int i = index; i < list.length; i++) {
			...
			if (currSum == goal)   {
				...
				subset_That_Sums_To_N(list, i + 1, currSum, goal, result + " " + list[i]);
			}
		}
	}
